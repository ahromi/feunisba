...
class Knpmk extends MX_Controller 
{
	...
	public function knpmk()
	{
		...
		{ 	
			 ...
			 $this->template->load('main_views','view_knpmk',$data);
		} else 
		{
			header('location:'.base_url());
		}
	...
	function inputKnpmk()
	{
		...
		}else
		{
			...
			$modul_dir		= opendir(dirname($_SERVER["SCRIPT_FILENAME"])."/uploads/".$pengguna."/knpmk");
				
			if(false == readdir($pengguna_dir)) //Jika Tidak ditemukan Directory Pengguna, Maka
			{
				mkdir(dirname($_SERVER["SCRIPT_FILENAME"])."/uploads/".$pengguna."/knpmk",0777,true); //Buat Directory Baru (Recursive), yakni dirctory pengguna dan directory modul
			}else if(false == readdir($modul_dir))												//Jika ditemukan directory pengguna, maka
			{
				mkdir(dirname($_SERVER["SCRIPT_FILENAME"])."/uploads/".$pengguna."/knpmk",0777);						//Hanya buat directory modul saja.
			}else 
			{
				closedir($modul_dir);
			}
			...
			//$namafile						= $cekpoto;
			$config['upload_path']		= dirname($_SERVER["SCRIPT_FILENAME"])."/uploads/".$pengguna."/knpmk";
			...
			else
			{
				...
				$nilaiDpna				= $this->security->xss_clean($this->input->post('nilai_dpna'));
				$nilaiTranskrip		= $this->security->xss_clean($this->input->post('nilai_transkrip'));
				$frs						= str_replace(' ', '_', $_FILES['files']['name'][0]);//Mengganti karakter spasi dengan underscores
				$transkrip				= str_replace(' ', '_', $_FILES['files']['name'][1]);//Mengganti karakter spasi dengan underscores
				$status					= '0';
				$tgl_pengajuan			= $created;
				
				$this->model_knpmk->saveKnpmk($npm,$matakuliah,$semester,$nilaiDpna,$nilaiTranskrip,$frs,$transkrip,$status,$tgl_pengajuan);
...
/* End of file knpmk.php */
/* Location: ./modules/knpmk/controllers/knpmk.php */