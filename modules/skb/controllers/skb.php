...
class Skb extends MX_Controller 
{
	...
	public function skb()
	{
		...
		{ 	
			 ...
			 $this->template->load('main_views','view_skb',$data);
		} else 
		{
			header('location:'.base_url());
		}
	...
	function skbAdmin()
	{
		...
		$hasil				= $this->model_skb->listSkb();
		$data['hasil']		= $hasil;
		
		//$data['coba']		= 'Isinya';
		$this->template->load('main_views','view_skb_admin',$data);
	}
	...
	function inputSkb()
	{
		...
		if(!$cekphoto) 
		{
			...
			header('location: '.base_url().'index.php/skb');
		}else
		{
			...
			$modul_dir		= opendir(dirname($_SERVER["SCRIPT_FILENAME"])."/uploads/".$pengguna."/skb");
				
			if(false == readdir($pengguna_dir)) //Jika Tidak ditemukan Directory Pengguna, Maka
			{
				mkdir(dirname($_SERVER["SCRIPT_FILENAME"])."/uploads/".$pengguna."/skb",0777,true); //Buat Directory Baru (Recursive), yakni dirctory pengguna dan directory modul
			}else if(false == readdir($modul_dir))												//Jika ditemukan directory pengguna, maka
			{
				mkdir(dirname($_SERVER["SCRIPT_FILENAME"])."/uploads/".$pengguna."/skb",0777);						//Hanya buat directory modul saja.
			}else 
			{
				closedir($modul_dir);
			}
			...
			$namafile						= $cekpoto;
			$config['upload_path']		= dirname($_SERVER["SCRIPT_FILENAME"])."/uploads/".$pengguna."/skb";
			...
			else
			{
				...
				$tahun_akademik		= $this->security->xss_clean($this->input->post('tahun_akademik'));
				
				$this->model_skb->saveSkb($npm,$tingkat,$semester,$tahun_akademik,$dpp,$status,$tgl_pengajuan);
	...
	function lihatDetailSkb()
	{
		...
		$hasil			= $this->model_skb->detailSkb($id);
		$data['hasil_cetak'] = $hasil;
		
		$this->template->load('main_views','view_detail_skb',$data);
	...
	function prosesSkb()
	{
		...
		$prosesSkb				= $this->model_skb->prosesSkb($id,$noSurat,$ttd,$status,$tgl_pemrosesan);
		if($prosesSkb)
		{
			$this->session->set_flashdata('sukses', 'Form SKB berhasil diproses');
			header('location: '.base_url().'index.php/skb');
		}else
		{
			$this->session->set_flashdata('gagal', 'Form SKB belum berhasil diproses, silakan ulangi kembali');
			header('location: '.base_url().'index.php/skb');
		}	
	}
	...
	function cetakSkb()
	{
		...
		$this->model_skb->updateSkb($id,$status,$tgl_pengesahan);
		$hasil			= $this->model_skb->cetakSkb($id);
		$data['hasil_cetak'] = $hasil;
		
	 	// page info here, db calls, etc.     
     $html = $this->load->view('view_cetak_skb', $data, true);
     pdf_create($html, 'filename');
	}
}

/* End of file skb.php */
/* Location: ./modules/skb/controllers/skb.php */